...
/*
 * vga_font_8x16.h — Standard VGA/IBM PC 8x16 bitmap font (CP437)
 *
 * Public domain font data from the IBM PC BIOS / Linux kernel fontdata_8x16.
 * Each character is 16 bytes (16 rows of 8 pixels), MSB = leftmost pixel.
 * Characters 0-255, but only 32-126 (printable ASCII) are typically used.
 *
 * This data matches the Linux kernel lib/fonts/font_8x16.c.
 */
...
static const uint8_t vga_font_8x16[256][16] = {
    ...
    /* 0x59 Y */
    {0x00,0x00,0xc6,0xc6,0xc6,0x6c,0x38,0x18,0x18,0x18,0x18,0x3c,0x00,0x00,0x00,0x00},  /* fixed: was 0x66 -> 0xc6 for consistent Y */
    /* 0x5a Z */
    ...
    /* 0x62 b */
    {0x00,0x00,0xe0,0x60,0x60,0x78,0x6c,0x66,0x66,0x66,0x66,0x7c,0x00,0x00,0x00,0x00},  /* fixed: was dc->7c */
    /* 0x63 c */
    ...
    /* 0x6d m */
    {0x00,0x00,0x00,0x00,0x00,0xe6,0xff,0xdb,0xdb,0xdb,0xdb,0xdb,0x00,0x00,0x00,0x00},  /* fixed: was ec->e6 */
    /* 0x6e n */
    ...
    /* 0x76 v */
    {0x00,0x00,0x00,0x00,0x00,0xc6,0xc6,0xc6,0xc6,0xc6,0x6c,0x38,0x00,0x00,0x00,0x00},  /* fixed: was 66->c6 */
    /* 0x77 w */
    ...
    /* 0x80 - 0xff: extended characters (CP437 box drawing, accented, etc.) */
    /* For brevity, these are set to the "missing glyph" pattern (stipple box) */
    ...
    /* 0x87 */ {0x00,0x00,0x00,0x00,0x3c,0x66,0x60,0x60,0x66,0x3c,0x0c,0x38,0x00,0x00,0x00,0x00},
    ...
    /* 0x91 */ {0x00,0x00,0x00,0x00,0x00,0x6e,0x3b,0x1b,0x7e,0xd8,0xdc,0x77,0x00,0x00,0x00,0x00},  /* fixed: was ec->6e */
    ...
    /* 0x9d */ {0x00,0x00,0xc6,0x6c,0x38,0x38,0xfe,0x38,0xfe,0x38,0x38,0x38,0x00,0x00,0x00,0x00},  /* fixed: was 66->c6 */
...
static void Terminal_dealloc(TerminalObject *self) {
    if (self->framebuf) framebuf_free(self->framebuf);
    if (self->atlas) atlas_free(self->atlas);
...
/* --- set_font_atlas(width, height, data: bytes) --- */

static PyObject *Terminal_set_font_atlas(TerminalObject *self, PyObject *args) {
    int glyph_w, glyph_h;
    const char *data;
    Py_ssize_t data_len;
    if (!PyArg_ParseTuple(args, "iiy#", &glyph_w, &glyph_h, &data, &data_len))
        return NULL;

    int num_glyphs = (int)(data_len / ((Py_ssize_t)glyph_w * glyph_h));
    if (num_glyphs <= 0) {
        PyErr_SetString(PyExc_ValueError, "Atlas data too small");
        return NULL;
    }

    /* Free old atlas and framebuffer */
    if (self->atlas) { atlas_free(self->atlas); self->atlas = NULL; }
    if (self->framebuf) { framebuf_free(self->framebuf); self->framebuf = NULL; }

    self->atlas = atlas_alloc(glyph_w, glyph_h, num_glyphs);
    if (!self->atlas) {
        PyErr_SetString(PyExc_MemoryError, "Failed to allocate glyph atlas");
        return NULL;
    }
    memcpy(self->atlas->data, data, (size_t)data_len);

    /* Allocate frame buffer sized for current terminal + padding */
    ScreenBuffer *scr = self->term->active;
    int fb_w = scr->cols * glyph_w;
    int fb_h = scr->rows * glyph_h;
    self->framebuf = framebuf_alloc(fb_w, fb_h);
    if (!self->framebuf) {
        atlas_free(self->atlas);
        self->atlas = NULL;
        PyErr_SetString(PyExc_MemoryError, "Failed to allocate frame buffer");
        return NULL;
    }
...
/* --- render_frame(fg="#ffffff", bg="#000000") -> bytes (PPM) --- */
...
static PyObject *Terminal_render_frame(TerminalObject *self, PyObject *args) {
    ...
    if (!self->atlas || !self->framebuf) {
        PyErr_SetString(PyExc_RuntimeError,
            "Font atlas not set — call set_font_atlas() first");
        return NULL;
    }

    /* Reallocate framebuf if terminal was resized */
    ScreenBuffer *scr = self->term->active;
    int need_w = scr->cols * self->atlas->glyph_w;
    int need_h = scr->rows * self->atlas->glyph_h;
    if (self->framebuf->width != need_w || self->framebuf->height != need_h) {
        framebuf_free(self->framebuf);
        self->framebuf = framebuf_alloc(need_w, need_h);
        if (!self->framebuf) {
            PyErr_SetString(PyExc_MemoryError, "Failed to reallocate frame buffer");
            return NULL;
        }
    }

    render_screen_to_framebuf(self->framebuf, scr, self->atlas, fg, bg);

    size_t ppm_len = 0;
    uint8_t *ppm = framebuf_to_ppm(self->framebuf, &ppm_len);
    if (!ppm) {
        PyErr_SetString(PyExc_MemoryError, "Failed to generate PPM");
        return NULL;
    }

    PyObject *result = PyBytes_FromStringAndSize((const char *)ppm, (Py_ssize_t)ppm_len);
    free(ppm);
    return result;
}
...
static PyMethodDef Terminal_methods[] = {
    ...
    {"set_font_atlas", (PyCFunction)Terminal_set_font_atlas, METH_VARARGS,
     "Set glyph atlas: set_font_atlas(glyph_w, glyph_h, data_bytes)."},
    {"render_frame",   (PyCFunction)Terminal_render_frame,   METH_VARARGS,
     "Render terminal to PPM bytes: render_frame(fg='#ffffff', bg='#000000')."},
